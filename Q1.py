...
# Thickness list (m) -> 1, 5, 20, 50μm
thickness_list = [1e-6, 5e-6, 20e-6, 50e-6]
thickness_labels = [1, 5, 20, 50]  # for labels (μm)
...
# --- MODIFICATION: Using a more aesthetically professional color palette and markers ---
# Professional color palette (Darker, modern, and distinct)
colors = ['#0072B2', '#D55E00', '#009E73', '#CC79A7'] # Dark Blue, Orange-Red, Green, Purple
linestyles = ['-', '--', '-.', ':']
...
plt.title(r'[Plot A] Emissivity vs. Wavelength of PDMS Thin Film (1-50μm) on Silver Substrate',
...
plt.savefig(r"D:\Python_Code\YaTaiBei\Data\PDMS_Emissivity_Plot_A_Styled.png", dpi=300, bbox_inches='tight')
plt.show()
...
# Define continuous thickness scan range (e.g., 0.1 to 60 μm)
thickness_scan = np.linspace(0.1e-6, 60e-6, 100)  # 100 thickness points
...
plt.title(r'[Plot B] Emissivity vs. Wavelength and Thickness (Heatmap)',
          fontsize=14, fontweight='bold')
plt.xlim(wl.min(), wl.max())
plt.ylim(thickness_scan_labels.min(), thickness_scan_labels.max())
plt.legend(loc='lower right')
plt.grid(True, alpha=0.2, linestyle=':')

# Save figure
plt.savefig(r"D:\Python_Code\YaTaiBei\Data\PDMS_Emissivity_Plot_B_Heatmap.png", dpi=300, bbox_inches='tight')
plt.show()
...
# Solar absorptivity should be low, set a smaller Y-limit to see changes
ax2.set_ylim(0, 0.1)

# Chart settings
plt.title(r'[Plot C] Key Performance Metrics vs. PDMS Thickness',
          fontsize=14, fontweight='bold')
plt.xlim(thickness_scan_labels.min(), thickness_scan_labels.max())
...
# Save figure
plt.savefig(r"D:\Python_Code\YaTaiBei\Data\PDMS_Emissivity_Plot_C_Average.png", dpi=300, bbox_inches='tight')
plt.show()
